...
import { setState, newState, changeTitle } from "../../store/store";
// axios
...
function addNewChat(){
  // ajax 요청
  axios.post(process.env.REACT_APP_LOCAL_SERVER_URL+'/addNewChat',null ,{ withCredentials: true })
  .then((result)=>{
  ...
  <div className='chat-list'>
    ...
    {/* member 새로운 채팅방 추가 기능 */}
    {
      storeData.userChatArr.length === 0 ? null : (storeData.userChatArr[0].id === null
        ...
        : <div className='chatroom-create-btn'><button onClick={addNewChat}><CiSquarePlus /><span>채팅방 추가</span></button></div>
      )
    }
